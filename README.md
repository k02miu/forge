...
| スキル | コマンド | 役割 | 使いどき |
|---|---|---|---|
| plan | `/forge:plan <要求文>` / `/forge:plan #N` | 新規 Issue 設計。`#N`(既存 Issue 番号/URL)指定時は改訂モードに切り替わり、現状の本文・議論を踏まえた改訂案を作る | 要件が曖昧、または影響範囲が広い変更に着手する前。既存 Issue の内容を練り直したいときの改訂にも使う |
| work | `/forge:work #<issue番号>` / `/forge:work "<作業内容>"` | Issue の実装。Issue 番号なしの自由文も受け付け、Issue を切るまでもない小規模修正に使える(完了後に事後 Issue 化を提案) | Issue があるとき。Issue を切るほどでもない小規模修正は自由文モードで直接実装に入れる |
| finish | `/forge:finish [--base <branch>]` | docs/テスト追随・静的チェック・収束型レビューループ | 実装が一段落し、ブランチの差分を仕上げたいとき |
| ship | `/forge:ship <PR番号または issue番号> [--auto] [--max-rounds N]` | PR 作成とレビュー対応ループ。既存 PR 番号を渡すと PR 作成をスキップし、レビュー対応ループへ直接入る | PR を作るとき、または既存 PR のレビュー対応を回すとき。`--auto` は commit & push の自動化のみで、対応方針の判断まで自動化するものではない |

典型フローは `plan → work → finish → ship`。途中参加も可能: 実装済みのブランチがあれば `finish` から、既に PR があれば `ship` から始めてよい。

finish は Claude Code 組み込みの `/code-review` と役割が重なるが、finish は複数観点(architecture/security/tests 等)を並列起動し、収束するまで Review → Verify → Fix を反復し、docs/テストの追随まで含む点が異なる。
