...
4. 曖昧点を `AskUserQuestion` で解消する(ワークフロー起動前のここでのみ使用可)。リポジトリから調べられる**事実**は質問せず自分で調べ、問うのは**意思決定**のみとする。各質問には推奨案を第一の選択肢として添える
...
| Design | `forge:analyst` | judge | zone ごとの実装仕様と criticalDecisions の策定。standard 以上かつ zone 数 2 以上のとき、`forge:analyst` による設計批評を 1 回追加し、blocking 時のみ改訂。fix タスクでは下記「fix タスクの診断規律」に従う。引き継ぎモードでは `gapAnalysis.remaining` のみを対象に計画する |
...
fix タスクの Design では testPlan / spec に次を必須で含める:

- 先に失敗する再現テストを書き、修正後に green を確認する。再現テストは**バグの症状そのもの**で fail すること(「エラーなく動く」の確認ではなく、このバグを捕まえられること)を要求する
- 再現は可能な限り最小化してから修正方針を立てる(残った要素がすべて失敗に寄与している状態が理想。最小化された再現がそのまま回帰テストになる)
- 根本原因の仮説は反証可能な形(「X が原因なら Y を変えると症状が変わる」)で挙げ、採用した仮説を spec に記す
- デバッグ用の一時計装を加える場合は一意のタグを付け、完了前の除去を testPlan に含める
...
let design = await agent(
  `Issue:\n${issue.body}\nzones:\n${JSON.stringify(ctx.zones)}\n${mode === 'handoff' ? `残作業のみ計画せよ(gapAnalysis.remaining): ${JSON.stringify(ctx.gapAnalysis.remaining)}\n` : ''}${taskType === 'fix' ? 'fixタスクの診断規律に従え: 先に失敗する再現テスト(バグの症状そのもので fail するもの)を書き、修正後にgreenを確認することをtestPlanに必須で含めよ。再現は最小化してから修正方針を立て、根本原因の仮説は反証可能な形で挙げて採用した仮説をspecに記せ。\n' : ''}知識ソース:\n${knowledgeBlocks.design}\nDESIGN スキーマで返せ。`,
